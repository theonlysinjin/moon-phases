...
export const CalendarGrid: React.FC<CalendarGridProps> = ({ moonPhases, theme, triggerRef, topTriggerRef, renderLoadPrevious }) => {
  ...
  if (theme === "poster-print") {
    // Poster-print: same layout as poster, but print-friendly (white background, dark text, light borders)
    const phaseByMonthDay: Record<number, Record<number, MoonPhaseEntry>> = {};
    moonPhases.forEach(entry => {
      const date = new Date(entry.date_utc.slice(0, 10));
      const month = date.getMonth() + 1; // 1-indexed
    ...
    const days = Array.from({ length: 31 }, (_, i) => i + 1);
    return (
      <div className="w-full overflow-x-auto mt-8 bg-white text-black">
        <table className="border-collapse mx-auto">
          <thead>
            <tr>
              <th className="w-8 h-8"></th>
              {months.map(month => (
                <th key={month} className="text-xs font-bold text-gray-800 px-2 py-1 text-center border-b border-gray-300">{month}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {days.map(day => (
              <tr key={day}>
                <td className="text-xs font-mono text-gray-600 text-right pr-2 align-middle">{day}</td>
                {months.map(month => {
                  const entry = phaseByMonthDay[month]?.[day];
                  return (
                    <td key={month + '-' + day} className="bg-white p-0 m-0 text-center align-middle border border-gray-200" style={{ width: 32, height: 32 }}>
                      {entry ? (
                        <div className="flex flex-col items-center justify-center" style={{ minHeight: 32, minWidth: 32 }}>
                          {getMoonPhaseVisual(entry, "1.25rem", "w-5 h-5 object-contain", theme)}
                        </div>
