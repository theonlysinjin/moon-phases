...
function parseMonthDay(str: string, year: number) {
  const dt = DateTime.fromFormat(`${str} ${year}`, 'd LLL yyyy', { zone: 'Africa/Johannesburg' });
  return dt;
}

type PhaseName = 'New Moon' | 'First Quarter' | 'Full Moon' | 'Last Quarter';

const TABLE_FILE = path.resolve(__dirname, './moon_phases_cape_town_2025.txt');

describe('Cape Town 2025 phases match local times (<= 2 minutes tolerance)', async () => {
  ...
  const generated = await getMajorMoonPhasesAstronomyEngine(2025, { timeZone: 'Africa/Johannesburg' });
  const genByPhase = generated.map(g => ({
    phase: g.phase as PhaseName,
    dt: DateTime.fromISO(`${g.date_local}T${g.time_local}:00`, { zone: 'Africa/Johannesburg' })
  }));
