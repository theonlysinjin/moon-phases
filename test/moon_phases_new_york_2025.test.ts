...
// Parse a tab-separated table line like:
// 1263\t29 Jan\t07:35\t5 Feb\t03:02\t12 Feb\t08:53\t20 Feb\t12:32\t29d 12h 09m
function parseLine(line: string) {
  const parts = line.split('\t').map(s => s.trim());
  if (!/^\d{4}$/.test(parts[0])) return null;
  const lunation = parts[0];
  ...
  return { lunation, phases: map };
}

function parseMonthDay(str: string, year: number) {
  // e.g. '29 Jan' -> '2025-01-29'
  const dt = DateTime.fromFormat(`${str} ${year}`, 'd LLL yyyy', { zone: 'America/New_York' });
  return dt;
}

type PhaseName = 'New Moon' | 'First Quarter' | 'Full Moon' | 'Last Quarter';

const TABLE_FILE = path.resolve(__dirname, './moon_phases_new_york_2025.txt');

describe('New York 2025 phases match local times (<= 2 minutes tolerance)', async () => {
  const text = fs.readFileSync(TABLE_FILE, 'utf-8');
  const lines = text.split(/\r?\n/).filter(l => l.trim().length > 0);
  // Extract data lines (skip headers)
  ...
  const generated = await getMajorMoonPhasesAstronomyEngine(2025, { timeZone: 'America/New_York' });
  const genByPhase = generated.map(g => ({
    phase: g.phase as PhaseName,
    dt: DateTime.fromISO(`${g.date_local}T${g.time_local}:00`, { zone: 'America/New_York' })
  }));
